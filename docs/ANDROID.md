...
v1.x 使用 Capacitor 封装 WebView，存在以下问题：

| 问题 | WebView (旧) | 原生 Kotlin (新) |
|------|-------------|-----------------|
| 启动速度 | ~2-3s (加载 Chromium + Web 资源) | ~200ms |
| 内存占用 | ~150MB+ | ~40-60MB |
| 推送可靠性 | WebView 进程被杀则收不到 | 系统级 FCM，独立于 App |
| 动画 | 偶有掉帧 | Compose 原生渲染 |
| 安全性 | JS 内存中私钥可被 dump | Android Keystore 硬件保护 |
| 通知 | 无法解密显示真实标题 | 可解密后显示完整内容 |
| 包大小 | ~15MB | ~8MB |

**技术栈**: Kotlin + Jetpack Compose + Material 3 + Hilt + Room + Retrofit + Bouncy Castle
...
2. **Android 包名**填写：`com.hxhoutiku.app`
   - 必须和 `android/app/build.gradle.kts` 中的 `applicationId` 一致
3. 应用昵称随意填
4. SHA-1 证书指纹**可以跳过**（FCM 不需要）
5. 点击 **注册应用**
6. 下载 `google-services.json`
7. **Base64 编码后存入 GitHub Secret**：
   ```bash
   base64 -w 0 google-services.json | gh secret set GOOGLE_SERVICES_JSON
   ```

> ⚠️ **这个文件绝对不要提交到 Git！** 它包含 Firebase 项目的 API Key。
> `.gitignore` 已忽略 `android/app/google-services.json`。
> CI 构建时由 GitHub Secrets (`GOOGLE_SERVICES_JSON`) 自动 base64 解码注入。
...
# google-services.json 从 GitHub Secret 解码到本地（一次性操作）
# 方法 A：用 gh CLI
gh secret list  # 确认 GOOGLE_SERVICES_JSON 存在
...
> 💡 **Android Studio 开发**: 直接用 Android Studio 打开 `android/` 目录即可，支持实时预览 Compose UI。
>
> 🔒 **安全设计**: `google-services.json` 和 `gradle.properties` 都在 `.gitignore` 中，
> 不会意外泄露。正式构建完全依赖 CI 的 GitHub Secrets 自动注入，开发者本地只需做一次性配置。
